...
RedBlackNode::RedBlackNode(RedBlackNode* _left, RedBlackNode* _right, RedBlackNode* _parent, RedBlackNodeColor _color,
    Employee* _data ) : left(_left), right(_right), parent(_parent), color(_color), data(_data)
{}
...
RedBlackNode* RedBlackTree::insertNode(Employee* data) {
...
RedBlackNode* RedBlackTree::findNode(Employee* data) {
...
void RedBlackTree::costMax_R(RedBlackNode* node, int& max)
{
    if (node != sentinel) {
        node->data->costMax(max);
        costMax_R(node->left, max);
        costMax_R(node->right, max);
    }
}

void RedBlackTree::longMax_R(RedBlackNode* node, int& max)
{
    if (node != sentinel) {
        node->data->longMax(max);
        longMax_R(node->left, max);
        longMax_R(node->right, max);
    }
}

int RedBlackTree::costMax()
{
    int cost = 0;
    costMax_R(root, cost);
...
int RedBlackTree::longMax()
{
    int longest = 0;
    longMax_R(root, longest);
